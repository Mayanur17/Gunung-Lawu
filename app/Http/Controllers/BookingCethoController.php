...
use App\Models\CethoDetail;
use App\Models\CethoAnggota;
use App\Models\CethoKuota;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Storage;

class BookingCethoController extends Controller
{
    public function form()
    {
        $booking = CethoDetail::where('user_id', Auth::id())->latest()->get();
        return view('pendaki.bookingcetho', compact('booking'));
    }
        ...
        public function simpan(Request $request)
{
    ...
    $kuota = CethoKuota::where('tanggal', $request->tanggal_pendakian)->first();
    if (!$kuota) {
    ...
    $total_terpakai = CethoDetail::where('tanggal_pendakian', $request->tanggal_pendakian)
    ...
    {
        $data = CethoDetail::with('anggota')->latest()->get();
        return view('admin.booking.cetho.index', compact('data'));
    }

    public function approve($id)
    {
        $booking = CethoDetail::findOrFail($id);
    ...
    public function cekKuota(Request $request)
    {
        ...
        $kuota = CethoKuota::where('tanggal', $tanggal)->first();
        $totalBooking = CethoDetail::where('tanggal_pendakian', $tanggal)->sum('jumlah_pendaki');
    ...
    {
        $booking = CethoDetail::with('anggota')->findOrFail($id);
        return view('admin.booking.cetho.detail', compact('booking'));
    }
