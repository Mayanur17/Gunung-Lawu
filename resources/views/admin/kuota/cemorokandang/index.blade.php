...
<div class="kuota-container">
    <h2>Atur Kuota Pendakian - Cemoro Kandang</h2>
        ...
        <tbody>
            @foreach($data as $item)
                <tr>
                    ...
                    <td>
                        <form method="POST" action="{{ route('kuota.cemorokandang.update', $item->id) }}">
                            @csrf
                    ...
                    <td>
                        <form method="POST" action="{{ route('kuota.cemorokandang.destroy', $item->id) }}">
                            @csrf
                            @method('DELETE')
