...
<div class="kuota-container">
    <h2>Atur Kuota Pendakian - Cemoro Sewu</h2>

    @if(session('success'))
        <div style="color: green;">{{ session('success') }}</div>
    @endif
    <form method="POST" action="{{ route('kuota.cemorosewu.store') }}" class="form-kuota ">
        @csrf
        ...
        <tbody>
            @foreach($data as $item)
                <tr>
                    ...
                    <td>
                        <form method="POST" action="{{ route('kuota.cemorosewu.update', $item->id) }}">
                            @csrf
                    ...
                    <td>
                        <form method="POST" action="{{ route('kuota.cemorosewu.destroy', $item->id) }}">
                            @csrf
                            @method('DELETE')
