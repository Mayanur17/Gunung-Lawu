...
class CreateCethoDetailTable extends Migration
{
    public function up()
    {
        Schema::create('cetho_detail', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('user_id'); 
            $table->date('tanggal_pendakian');
            $table->date('tanggal_turun');
            $table->enum('keterangan', ['Tektok', 'Camp']);
            $table->string('jalur')->default('Cetho'); 
    ...
    {
        Schema::dropIfExists('cetho_detail');
    }
}
